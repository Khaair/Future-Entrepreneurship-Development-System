...
class Manageslider extends Component {
  ...
  // };
  Changesqoute = (e, data) => {
    this.setState({ sliderqoute: data.getData() });
  };
  onSubmit = async (event) => {
    event.preventDefault();
    const { Image, slidertitle, sliderqoute } = this.state;
    ...
    if (!Image) {
      return alert("Give a Slder image!");
    }
    if (!slidertitle) {
      return alert("Give a Slder Title!");
    }
    if (!sliderqoute) {
      return alert("Give a Slder Qoute!");
    }
    ...
    await axios.post(`${URL}/slidersetting`, variables).then((response) => {
      console.log(response);
      if (response.data.success) {
        this.componentDidMount();
        this.setState({ slidertitle: "", sliderqoute: "", Image: "" });
        alert("Slider Succesfully added");
      } else {
        ...
        <div id="page-content-wrapper">
          ...
          <div className="container-fluid">
            ...
            <form onSubmit={this.onSubmit}>
              ...
              <div className="form-group">
                <label htmlFor="exampleInputEmail1">Slider Qoute</label>

                <CKEditor
                  editor={ClassicEditor}
                  value={this.state.sliderqoute}
          ...
          <div className="container-fluid">
            <h1 className="mt-4">Your Sliders</h1>
                ...
                {this.state.sliders.map((slider, index) => {
                  return (
                    <tr key={index}>
                      ...
                      <td data-label="Title">{slider.slidertitle}</td>
                      <td data-label="Qoute">
                        {ReactHtmlParser(slider.sliderqoute)}
                      </td>
                      <td data-label="Image">
                        <img
                      ...
                      <td data-label="Action">
                        <button
                          type="button"
                          onClick={() => this.remove(slider._id)}
                        >
                          Remove
                        </button>
                      </td>
